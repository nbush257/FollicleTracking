...
for qq = 1:length(tifNumbers)   %% loop through the Sections
    ...
    if ~isempty(s)
        for ii = 1:length(s)
            ...
            mf = data(end-5:end-4);
            if strcmp(mf,'ed') == 1
                RemoveMe = [RemoveMe,ii];
            elseif strcmp(mf,'ed') == 0
  ...
  s([RemoveMe])= [];
    ...
    while folNumber <= nFollicles
        ...
        tempstring = ['save ' fname ' EllipsesO EllipsesI OuterRawPts InnerRawPts nFollicles  BadOuter BadInner'];
        disp(tempstring); eval(tempstring);
        
        folNumber = folNumber + 1;
        
    end % while loop on each of the follicles
    disp('Press any key if you''re happy with this section');
    pause;
    %%% at the start of the next section prompt the user to input
    %%% nFollicles in command window.
    fig2;clf;text(1,1,'input N follicles in command window'); axx(-1,2);axy(-1,2);
